...
const NavBar = () => {
  ...
  const navLinks = ["HOME", "LAPTOPS", "GAMING", "ACCESSORIES", "OFFERS"];
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/product/search?brand=${encodeURIComponent(searchQuery)}`
      );
      if (response.ok) {
                  ...
                  {searchResults.map((product) => (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-amber-500">RS: {product.price}</div>
                      </div>
                    </a>
                  ))}
                </div>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden px-4 py-4 space-y-4 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700">
          ...
          <div className="flex gap-4 pt-2">
            {!isLoggedIn ? (
              <>
                <button className="text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-amber-500" onClick={handleLogin}>Login</button>
                <button className="text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-amber-500" onClick={handleRegister}>Register</button>
              </>
            ) : (
              <button className="text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-amber-500" onClick={handleLogout}>Logout</button>
            )}
          </div>
